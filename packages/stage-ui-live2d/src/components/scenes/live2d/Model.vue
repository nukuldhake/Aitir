...
import {
  ...
  useMotionUpdatePluginEmotionAnimation,
...
const emotionAnimator = createEmotionAnimator()
...
async function loadModel() {
  ...
  try {
    ...
    // NOTICE: Diagnostic log to verify capabilities of new models
    console.warn('[Model] Available Motions:', availableMotions.value)
    const paramIds = (coreModel as any)._parameterIds || Object.keys((coreModel as any)._parameters || {})
    console.warn('[Model] Available Parameter IDs:', paramIds)
    ...
    motionManagerUpdate.register(useMotionUpdatePluginEmotionAnimation(emotionAnimator), 'post')
...
async function setMotion(motionName: string, index?: number) {
  ...
  const definitions = motionManager.definitions as Record<string, unknown[] | undefined>
  const groupNames = Object.keys(definitions)

  // Resolve the motion group name against what the model actually has
  let resolvedGroup = motionName

  if (!definitions[motionName]) {
    // Try case-insensitive exact match first
    const ciMatch = groupNames.find(g => g.toLowerCase() === motionName.toLowerCase())
    if (ciMatch) {
      resolvedGroup = ciMatch
    }
    else {
      // Try partial match (e.g., "hap" matches "Happy", "Think" matches "Thinking")
      const partialMatch = groupNames.find(g =>
        g.toLowerCase().includes(motionName.toLowerCase())
        || motionName.toLowerCase().includes(g.toLowerCase()),
      )
      if (partialMatch) {
        resolvedGroup = partialMatch
      }
      else {
        // No match found — fall back to Idle
        resolvedGroup = groupNames.find(g => g.toLowerCase() === 'idle') || groupNames[0] || motionName
      }
    }
  }

  try {
    await model.value.motion(resolvedGroup, index, MotionPriority.FORCE)
  }
  catch (error) {
...
// Watch for emotion changes from the store and trigger parameter-based animations
watch(() => live2dStore.activeEmotion, (emotion) => {
  if (emotion) {
    const preset = getEmotionPreset(emotion.name)
    emotionAnimator.play(preset, emotion.intensity)
  }
})
