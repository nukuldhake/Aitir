...
const VS_DECL = `
#ifndef SAKURA_DIFFUSE_VS_DECL
#define SAKURA_DIFFUSE_VS_DECL
varying vec3 vWorldNormal;
#endif
`

const VS_APPLY = `
#ifndef SAKURA_DIFFUSE_VS_APPLY
#define SAKURA_DIFFUSE_VS_APPLY
vWorldNormal = normalize( mat3( modelMatrix ) * objectNormal );
#endif
`

const FS_COMMON = `
#ifndef SAKURA_DIFFUSE_COMMON
#define SAKURA_DIFFUSE_COMMON
...
vec3 SAKURA_evalIrradianceSH(vec3 n){
  n = normalize(n);
...
const FS_APPLY = `
#ifndef SAKURA_DIFFUSE_APPLY
#define SAKURA_DIFFUSE_APPLY
if (uNprEnvMode == 2) {
  vec3 I = SAKURA_evalIrradianceSH(normalize(vWorldNormal));
...
export function injectDiffuseIBL(mat: THREE.ShaderMaterial) {
  ...
  mat.customProgramCacheKey = () => `${baseKey}|SAKURA-diffuse-ibl`
  ...
  mat.onBeforeCompile = (shader: any, renderer: any) => {
    ...
    if (!shader.vertexShader.includes('SAKURA_DIFFUSE_VS_DECL')) {
      shader.vertexShader = `${VS_DECL}\n${shader.vertexShader}`
    }
    if (shader.vertexShader.includes('#include <defaultnormal_vertex>')
      && !shader.vertexShader.includes('SAKURA_DIFFUSE_VS_APPLY')) {
    ...
    if (!shader.fragmentShader.includes('SAKURA_DIFFUSE_COMMON')) {
    ...
    if (!shader.fragmentShader.includes('SAKURA_DIFFUSE_APPLY')) {
      shader.fragmentShader = shader.fragmentShader.replace(
    ...
    (mat.userData ||= {}).__SAKURAIbl = shader.uniforms
  }
...
export function updateNprShaderSetting(
  ...
  root.traverse((o) => {
    ...
    mats.forEach((m) => {
      const u = m?.userData?.__SAKURAIbl
      if (!u)
        return
